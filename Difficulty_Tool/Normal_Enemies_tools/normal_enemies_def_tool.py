...
def_factor = input('Multiply Normal Enemies Defense by : ')
...
for item in data.values():
    ...
    if item_class == 'ClassWithId':
        ...
        def_monster = item[1]['Values'][5][1]
        if monster_id <= 1500:  # Check if Monster is Regular/Not Boss
            def_monster = float(def_monster)
            def_monster = def_monster * def_factor
            def_monster = int(def_monster)
            item2 = item
            item2[1]['Values'][5][1] = def_monster
...
print("""
You Change all Normal Enemies Defense by x""" + display_def_factor + """ !!
Enjoy!
""")
