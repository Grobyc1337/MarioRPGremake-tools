...
sp_def_factor = input('Multiply Normal Enemies Sp.Defense by : ')
...
for item in data.values():
    ...
    if item_class == 'ClassWithId':
        ...
        sp_def_monster = item[1]['Values'][7][1]
        if monster_id <= 1500:  # Check if Monster is Regular/Not Boss
            sp_def_monster = float(sp_def_monster)
            sp_def_monster = sp_def_monster * sp_def_factor
            sp_def_monster = int(sp_def_monster)
            item2 = item
            item2[1]['Values'][7][1] = sp_def_monster
...
print("""
You Change all Normal Enemies Sp.Defense by x""" + display_sp_def_factor + """ !!
Enjoy!
""")
