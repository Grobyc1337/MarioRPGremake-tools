...
speed_factor = input('Multiply Normal Enemies Speed by : ')
...
for item in data.values():
    ...
    if item_class == 'ClassWithId':
        ...
        speed_monster = item[1]['Values'][8][1]
        if monster_id <= 1500:  # Check if Monster is Regular/Not Boss
            speed_monster = float(speed_monster)
            speed_monster = speed_monster * speed_factor
            speed_monster = int(speed_monster)
            item2 = item
            item2[1]['Values'][8][1] = speed_monster
...
print("""
You Change all Normal Enemies Speed by x""" + display_speed_factor + """ !!
Enjoy!
""")
