...
atk_factor = input('Multiply Boss Enemies Attack by : ')
display_atk_factor = atk_factor

try:
    val = float(atk_factor)
except ValueError:
    print("That's not an int!")
    sys.exit()

atk_factor = float(atk_factor)

for item in data.values():
    item_class = item[0]
    if item_class == 'ClassWithId':
        monster_id = item[1]['Values'][1][1]
        atk_monster_boss = item[1]['Values'][4][1]
        if monster_id >= 1500:  # Check if Monster is a Boss
            atk_monster_boss = float(atk_monster_boss)
            atk_monster_boss = atk_monster_boss * atk_factor
            atk_monster_boss = int(atk_monster_boss)
            item2 = item
            item2[1]['Values'][4][1] = atk_monster_boss
...
print("""
You Change all Boss Enemies Attack by x""" + display_atk_factor + """ !!
Enjoy!
""")
