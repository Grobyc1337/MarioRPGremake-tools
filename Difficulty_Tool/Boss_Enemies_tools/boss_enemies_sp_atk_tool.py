...
sp_atk_factor = input('Multiply Boss Enemies Sp.Attack by : ')
display_sp_atk_factor = sp_atk_factor

try:
    val = float(sp_atk_factor)
except ValueError:
    print("That's not an int!")
    sys.exit()

sp_atk_factor = float(sp_atk_factor)

for item in data.values():
    item_class = item[0]
    if item_class == 'ClassWithId':
        monster_id = item[1]['Values'][1][1]
        spa_atk_monster_boss = item[1]['Values'][6][1]
        if monster_id >= 1500:  # Check if Monster is Regular/Not Boss
            spa_atk_monster_boss = float(spa_atk_monster_boss)
            spa_atk_monster_boss = spa_atk_monster_boss * sp_atk_factor
            spa_atk_monster_boss = int(spa_atk_monster_boss)
            item2 = item
            item2[1]['Values'][6][1] = spa_atk_monster_boss
...
print("""
You Change all Boss Enemies Sp.Attack by x""" + display_sp_atk_factor + """ !!
Enjoy!
""")
