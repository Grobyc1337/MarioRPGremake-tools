...
hp_factor = input('Multiply Bosses enemies HP by : ')
...
for item in data.values():
    ...
    if item_class == 'ClassWithId':
        ...
        hp_monster_boss = item[1]['Values'][2][1]
        if monster_id >= 1500:  # Check if Monster is a Boss
            hp_monster_boss = float(hp_monster_boss)
            hp_monster_boss = hp_monster_boss * hp_factor
            hp_monster_boss = int(hp_monster_boss)
            item2 = item
            item2[1]['Values'][2][1] = hp_monster_boss
...
print("""
You Change all Boss HP's by x""" + display_hp_factor + """ !!
Enjoy!
""")
