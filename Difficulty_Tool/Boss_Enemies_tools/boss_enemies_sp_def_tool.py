...
sp_def_factor = input('Multiply Boss Enemies Sp.Defense by : ')
display_sp_def_factor = sp_def_factor

try:
    val = float(sp_def_factor)
except ValueError:
    print("That's not an int!")
    sys.exit()

sp_def_factor = float(sp_def_factor)

for item in data.values():
    item_class = item[0]
    if item_class == 'ClassWithId':
        monster_id = item[1]['Values'][1][1]
        spa_def_monster_boss = item[1]['Values'][7][1]
        if monster_id >= 1500:  # Check if Monster is Regular/Not Boss
            spa_def_monster_boss = float(spa_def_monster_boss)
            spa_def_monster_boss = spa_def_monster_boss * sp_def_factor
            spa_def_monster_boss = int(spa_def_monster_boss)
            item2 = item
            item2[1]['Values'][7][1] = spa_def_monster_boss
...
print("""
You Change all Boss Enemies Sp.Defense by x""" + display_sp_def_factor + """ !!
Enjoy!
""")
