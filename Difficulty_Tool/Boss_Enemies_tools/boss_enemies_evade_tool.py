...
evade_factor = input('Multiply Boss Enemies Evade by : ')
display_evade_factor = evade_factor

try:
    val = float(evade_factor)
except ValueError:
    print("That's not an int!")
    sys.exit()

evade_factor = float(evade_factor)

for item in data.values():
    item_class = item[0]
    if item_class == 'ClassWithId':
        monster_id = item[1]['Values'][1][1]
        evade_monster_boss = item[1]['Values'][9][1]
        if monster_id >= 1500:  # Check if Monster is Regular/Not Boss
            evade_monster_boss = float(evade_monster_boss)
            evade_monster_boss = evade_monster_boss * evade_factor
            evade_monster_boss = int(evade_monster_boss)
            item2 = item
            item2[1]['Values'][9][1] = evade_monster_boss
...
print("""
You Change all Boss Enemies Evade by x""" + display_evade_factor + """ !!
Enjoy!
""")
