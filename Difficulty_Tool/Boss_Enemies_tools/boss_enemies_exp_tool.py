...
exp_factor = input('Multiply Boss Enemies Exp by : ')
display_exp_factor = exp_factor

try:
    val = float(exp_factor)
except ValueError:
...
for item in data.values():
    ...
    if item_class == 'ClassWithId':
        ...
        exp_monster_boss = item[1]['Values'][23][1]
        if monster_id >= 1500:  # Check if Monster is Regular/Not Boss
            exp_monster_boss = float(exp_monster_boss)
            exp_monster_boss = exp_monster_boss * exp_factor
            exp_monster_boss = int(exp_monster_boss)
            item2 = item
            item2[1]['Values'][23][1] = exp_monster_boss
...
print("""
You Change all Boss Enemies Exp by x""" + display_exp_factor + """ !!
Enjoy!
""")
