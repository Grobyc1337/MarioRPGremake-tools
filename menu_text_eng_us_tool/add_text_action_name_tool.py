...
while active:
    ...
    if last_skill_id == 750:   # Means list has Original length -> Use 50000 for new offset
        ...
        list_length_fix = 1  # Set List Length for new List
    else:
        id_missing = False
        last_skill_id = last_skill_id - 1  # Offset by one for Skill #750
    ...
    if amount_to_add > j:  # j = 1  here
        ...
        for item2 in the_list_2:
            ...
            if item_ba_class == 'BinaryArray':
                ...
                check_highest_ba_id.append(object_ba_id)
                if object_ba_id == 14:
                    item2[1]['Lengths'][0] = item2[1]['Lengths'][0] + amount_to_add + list_length_fix  # + 1 Fixed New Length
    ...
    # ~~~~~~~~  If input is 1   -> prompt "enter custom name" ~~~~~~~~
        ...
        for item_ba in the_list_2:
            ...
            if item_ba_class == 'BinaryArray':
                ...
                if object_ba_id == 14:
                    if new_list:
                        item_ba[1]['Lengths'][0] = item_ba[1]['Lengths'][0] + amount_to_add + 1  # + 1 Fixed New Length
                    else:
                        item_ba[1]['Lengths'][0] = item_ba[1]['Lengths'][0] + amount_to_add  # If not new list
                if object_ba_id == 16:
